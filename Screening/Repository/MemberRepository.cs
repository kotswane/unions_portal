...
{
    public class MemberRepository<T> : IasyncRepository<T> where T : class
    {
        ...
        {
            return await _context.BranchMember.Where(m => m.MemberBranchId == branchId).Include(m => m.Employer).Include(m => m.Branch).ToListAsync();
        }
    }
}
